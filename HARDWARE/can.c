...
void CAN1_RX0_IRQHandler(void)
{
	...
	//////////////// 预留给用户的CAN接口操作模板，用于控制机器人运动 ////////////
//	else if (id==0x???)
//	{
//		if(解锁CAN控制条件满足) 
//			Set_Control_Mode(_CAN_Control); //设置CAN控制模式
//		
//		if(Get_Control_Mode(_CAN_Control)) //处于CAN控制模式，则接收CAN的目标速度
//		{
//			disable_robot_count=0;          //数据检查计时清0
//			Move_X = xxx;                   //接收X轴速度
//			Move_Y = xxx;                   //接收Y轴速度
//			Move_Z = xxx;                   //接收Z轴速度
//		}
//	}
	...
	if(id==0x182)
	{
		...
		Recharge_VOL=((float)((short)((temp_rxbuf[2]<<8)+(temp_rxbuf[3]))))/10;
	...
	/*************** 以下是S300/S150机器人才有的数据 ***************/
	
	//////////////// 超声波数据 ////////////////
//	else if(id==0x101)
//	{
//		CheckDistance++;
//		ultrasonic.A = ((float)((short) ((temp_rxbuf[0]<<8)|(temp_rxbuf[1]))))/1000;
//		ultrasonic.B = ((float)((short) ((temp_rxbuf[2]<<8)|(temp_rxbuf[3]))))/1000;
//		ultrasonic.C = ((float)((short) ((temp_rxbuf[4]<<8)|(temp_rxbuf[5]))))/1000;
//		ultrasonic.D = ((float)((short) ((temp_rxbuf[6]<<8)|(temp_rxbuf[7]))))/1000;
//	}
//	else if(id==0x102)
//	{
//		CheckDistance++;
//		ultrasonic.E = ((float)((short) ((temp_rxbuf[0]<<8)|(temp_rxbuf[1]))))/1000;
//		ultrasonic.F = ((float)((short) ((temp_rxbuf[2]<<8)|(temp_rxbuf[3]))))/1000;
//	}
	//////////////// 超声波数据 ////////////////

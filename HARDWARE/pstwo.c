...
void PS2_Read(void)
{
  ...
  //读取右边遥感Y轴方向的模拟量  
	...
	if(PS2_KEY==4&&Get_Control_Mode(_PS2_Control)==0) 
		//The start button on the // handle is pressed
		//手柄上的Start按键被按下
		Strat=1; 
	
	if(Strat&&(PS2_LY<118)&&Get_Control_Mode(_PS2_Control)==0&&Deviation_Count>=CONTROL_DELAY)
...
{
	if(Get_Control_Mode(_PS2_Control))
		{
		PS2_LX=PS2_AnologData(PSS_LX);
		PS2_LY=PS2_AnologData(PSS_LY);
		PS2_RX=PS2_AnologData(PSS_RX);

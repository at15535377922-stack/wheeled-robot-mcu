...
{	
	...
	//hub_485_torque_init();
	hub_CAN_velocity_init();//轮毂电机速度模式初始化
	//hub_CAN_position_init();
	//hub_CAN_torque_init();
	//hub_CAN_Rpm(1, 50);
	hub_CAN_Encoder_init();//映射编码器数据
	delay_ms(500);
	
	hub_MotorTemperature_Init();//映射电机、驱动温度
	delay_ms(50);
	hub_MotorCurrent_Init();//映射电机电流、母线电压
	delay_ms(50);
	hub_mainParam_set();//对驱动器的重要参数重新设置
	delay_xms(100);
	
	motor_checkerror_flag=1;//检查驱动器是否报错
	CAN_CheckError();
	delay_ms(100);
	if(motor_clear_error)
	{
		...
		CAN_Enable();//重新使能电机
		delay_ms(1);
		hub_CAN_Syn_Rpm(0,0);//设置电机速度为0防止机器人运动
		motor_clear_error=0;
	}
}
/**************************************************************************

...
void start_task(void *pvParameters)
{
	...
	if(Car_Mode==S300||Car_Mode==S150||Car_Mode==S100||Car_Mode==S200)
		xTaskCreate(ReadUS_task,  "ReadUS_task",  US_STK_SIZE,      NULL, US_TASK_PRIO,      NULL);	//Read the Ultrasonic data task //读取超声波并计算距离任务
	
	xTaskCreate(Get_MotorState_task,"Get_MotorState_task",MotorState_STK_SIZE,NULL,MotorState_TASK_PRIO,NULL);//自检状态发送任务
